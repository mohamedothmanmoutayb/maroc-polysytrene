...
@section('title', 'Sortie de Production - Finale')
            ...
            <div class="row align-items-center">
                <div class="col-12">
                    <div class="d-sm-flex align-items-center justify-space-between">
                        <h4 class="mb-4 mb-sm-0 card-title">Sortie de Production - Finale</h4>
                        <nav aria-label="breadcrumb" class="ms-auto">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                                <li class="breadcrumb-item"><a href="{{ route('production-orders.index') }}">Ordres de
                                        Production</a></li>
                                <li class="breadcrumb-item active">Sortie Finale</li>
                            </ol>
                        </nav>
                    </div>
                </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            const sourceProducts = {!! json_encode($sourceProducts ?? []) !!};
            ...
            function loadOrderDetails() {
                $('#orderInfoContent').html(
                    '<div class="text-center py-4"><div class="spinner-border text-success"></div><p>Chargement...</p></div>'
                );
            ...
            function displayOrderInfo(order) {
                const conversionDetails = order.conversion_details || {};

                let sourcesHtml = '';
                if (sourceProducts && sourceProducts.length > 0) {
                    sourcesHtml = sourceProducts.map(sp =>
                        `<div class="d-flex justify-content-between py-1 border-bottom">
                            <span>${sp.product_name}</span>
                            <span class="badge bg-secondary">${sp.quantity} u</span>
                        </div>`
                    ).join('');
                }
            ...
            <div class="order-info-card">
                ...
                <div class="order-info-row">
                    ...
                    <span class="order-info-value"><span class="badge bg-success">${order.source_famille_name || order.conversion_details?.source_famille_name || 'Non spécifié'}</span></span>
                </div>
                ${sourcesHtml ? `
                <div class="mt-3">
                    <div class="fw-semibold mb-2" style="color:#495057">Produits Sources:</div>
                    ${sourcesHtml}
                </div>` : `
                <div class="order-info-row">
                    <span class="order-info-label">Produit Source:</span>
                    <span class="order-info-value"><strong>${conversionDetails.source_product_name || 'N/A'}</strong></span>
                </div>`}
                <div class="mt-3">
                    <div class="progress">
                        <div id="orderProgressBar" class="progress-bar bg-success" style="width: 0%"></div>
                    </div>
                </div>
            </div>
        `;
            ...
            function updateCalculations() {
                ...
                $('.product-row').each(function() {
                    ...
                    const conversionRate = parseFloat($(this).data('conversion-rate')) || 1;
        ...
        `);
            ...
            $('#productionOutputForm').submit(function(e) {
                ...
                $('.product-row').each(function() {
                    ...
                    totalSousBlocsToConsume += quantity * conversionRate;
                ...
                $.ajax({
                    url: "{{ route('production-output.store-type3') }}",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(response) {
                        if (response.success) {
                            ...
                            submitBtn.prop('disabled', false).html(
                                '<i class="fas fa-save me-2"></i> Enregistrer la Conversion'
                            );
                        }
                    },
                    error: function(xhr) {
