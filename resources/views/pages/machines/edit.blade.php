...
@section('title', 'Modifier Machine - ' . $machine->name)
            ...
            <div class="row align-items-center">
                <div class="col-12">
                    <div class="d-sm-flex align-items-center justify-space-between">
                        <h4 class="mb-4 mb-sm-0 card-title">Modifier Machine</h4>
        ...
        <div class="card">
            <div class="card-header card-header-custom">
                <h5 class="card-title mb-0" style="color:white">
                    <i class="fas fa-edit me-2"></i>Modifier la Machine
                </h5>
            </div>
            <div class="card-body">
                <form id="machineForm">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="model" value="{{ $machine->model }}">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="purchase_date"
                                    value="{{ $machine->purchase_date ? $machine->purchase_date->format('Y-m-d') : '' }}">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="form-label">Heures de fonctionnement</label>
                                <input type="number" class="form-control" name="operating_hours"
                                    value="{{ $machine->operating_hours }}" min="0">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="status" required>
                                    <option value="active" {{ $machine->status == 'active' ? 'selected' : '' }}>Actif
                                    </option>
                                    <option value="maintenance" {{ $machine->status == 'maintenance' ? 'selected' : '' }}>
                                        En maintenance</option>
                                    <option value="inactive" {{ $machine->status == 'inactive' ? 'selected' : '' }}>Inactif
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <div id="documentsContainer">
                                @foreach ($documentTypes as $type)
                                    @php
                                        $currentDoc = $machine->getCurrentDocument($type->document_type_id);
                                    @endphp
                    ...
                    <div class="mb-4">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="notes" rows="3">{{ $machine->notes }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-2 mt-4">
                        ...
                        <a href="{{ route('machines.show', $machine->machine_id) }}" class="btn btn-info">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#machineForm').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('machines.update', $machine->machine_id) }}",
                    type: 'POST',
                    data: $(this).serialize(),
                    success: function(response) {
                        if (response.success) {
                            ...
                            setTimeout(function() {
                                window.location.href =
                                    "{{ route('machines.show', $machine->machine_id) }}";
                            }, 1500);
                        } else {
