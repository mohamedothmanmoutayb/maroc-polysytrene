...
@section('title', 'Modifier Chauffeur - ' . $driver->full_name)
            ...
            <div class="row align-items-center">
                <div class="col-12">
                    <div class="d-sm-flex align-items-center justify-space-between">
                        <h4 class="mb-4 mb-sm-0 card-title">Modifier Chauffeur</h4>
        ...
        <div class="card">
            <div class="card-header card-header-custom">
                <h5 class="card-title mb-0" style="color:white">
                    <i class="fas fa-edit me-2"></i>Modifier les informations du chauffeur
                </h5>
            </div>
            <div class="card-body">
                <form id="driverForm">
                                    ...
                                    <select class="form-control select2-employee" name="employee_id" required
                                        ...
                                        @foreach ($employees as $employee)
                                            <option value="{{ $employee->employee_id }}"
                                                {{ $driver->employee_id == $employee->employee_id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <select class="form-control" name="license_category">
                                            <option value="A" {{ $driver->license_category == 'A' ? 'selected' : '' }}>
                                                A - Moto</option>
                                            <option value="B"
                                                {{ $driver->license_category == 'B' ? 'selected' : '' }}>B - Voiture
                                            </option>
                                            <option value="C"
                                                {{ $driver->license_category == 'C' ? 'selected' : '' }}>C - Camion
                                            </option>
                                            <option value="D"
                                                {{ $driver->license_category == 'D' ? 'selected' : '' }}>D - Transport de
                                                personnes</option>
                                            <option value="E"
                                                {{ $driver->license_category == 'E' ? 'selected' : '' }}>E - Remorque
                                            </option>
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <small class="text-muted">
                                            @if ($driver->license_expiring_soon)
                                                <span class="text-warning">⚠️ Expire bientôt!</span>
                                            @elseif($driver->license_expiry_date && $driver->license_expiry_date < now())
                                                <span class="text-danger">⚠️ Permis expiré!</span>
                                            @endif
                                        </small>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="date" class="form-control" name="medical_visit_date"
                                            value="{{ old('medical_visit_date', $driver->medical_visit_date ? $driver->medical_visit_date->format('Y-m-d') : '') }}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="date" class="form-control" name="next_medical_visit_date"
                                            value="{{ old('next_medical_visit_date', $driver->next_medical_visit_date ? $driver->next_medical_visit_date->format('Y-m-d') : '') }}">
                                        @if ($driver->medical_visit_due_soon)
                                            <small class="text-warning">⚠️ Visite médicale prévue bientôt!</small>
                                        @elseif($driver->next_medical_visit_date && $driver->next_medical_visit_date < now())
                                            <small class="text-danger">⚠️ Visite médicale en retard!</small>
                                        @endif
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="status" required>
                                    <option value="active" {{ $driver->status == 'active' ? 'selected' : '' }}>Actif
                                    </option>
                                    <option value="inactive" {{ $driver->status == 'inactive' ? 'selected' : '' }}>Inactif
                                    </option>
                                    <option value="suspended" {{ $driver->status == 'suspended' ? 'selected' : '' }}>
                                        Suspendu</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="notes" rows="3">{{ old('notes', $driver->notes) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-2 mt-4">
                        ...
                        <a href="{{ route('drivers.show', $driver->driver_id) }}" class="btn btn-secondary">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#driverForm').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('drivers.update', $driver->driver_id) }}",
                    type: 'POST',
                    data: $(this).serialize(),
                    success: function(response) {
                        if (response.success) {
                            ...
                            setTimeout(function() {
                                window.location.href =
                                    "{{ route('drivers.show', $driver->driver_id) }}";
                            }, 1500);
                        } else {
                            showToast('error', response.message);
                            submitBtn.prop('disabled', false).html(originalText);
